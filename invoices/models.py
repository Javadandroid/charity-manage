...
class Invoice(models.Model):
    ...
    def save(self, *args, **kwargs):
        # ایجاد شماره فاکتور اگر وجود نداشته باشد
        if not self.invoice_number:
            # دریافت آخرین شماره فاکتور و افزایش آن
        ...
        if self.receipt_image and hasattr(self.receipt_image, 'path') and os.path.exists(self.receipt_image.path):
            ...
            # ذخیره مجدد بدون فراخوانی recursion
    ...
    @property
    def subtotal(self):
        """محاسبه جمع آیتم‌ها بدون تخفیف و همت عالی"""
        return sum(item.total_price for item in self.items.all())
    
    def formatted_total(self):
        """نمایش مبلغ کل با فرمت مناسب"""
...
class InvoiceItem(models.Model):
    ...
    def formatted_total(self):
        """نمایش قیمت کل با فرمت مناسب"""
        return f"{self.total_price:,} تومان"
    
    def save(self, *args, **kwargs):
        # اگر قیمت وارد نشده باشد، از قیمت محصول استفاده شود
        if not self.price:
