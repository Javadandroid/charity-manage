...
def booth_dashboard(request, pk):
    ...
    card_sales = Invoice.objects.filter(
        booth=booth, payment_method='card').aggregate(
        Sum('total_amount'))['total_amount__sum'] or 0
    cash_sales = Invoice.objects.filter(
        booth=booth, payment_method='cash').aggregate(
        Sum('total_amount'))['total_amount__sum'] or 0
    ...
    try:
        ...
        if pos_device.bank == 'PASARGAD' and pos_device.connection_type == 'TCP':
            ...
            if not ip or not port:
                ...
                client = PosPasargadClient(
                    ip=str(ip), port=int(port), timeout_ms=getattr(
                        settings, 'POS_TIMEOUT_MS', 30000), debug=getattr(
                        settings, 'POS_DEBUG', False))
    ...
    if result:
        ...
        if is_ajax:
            return JsonResponse({'ok': True,
                                 'message': str(_('پرداخت با موفقیت انجام شد.')),
        ...
        if is_ajax:
            return JsonResponse({'ok': False, 'message': _(
                'خطا در پرداخت: {0}').format(error_message)}, status=502)
...
def send_to_pos_tcp(pos_device, amount):
    ...
    if pos_device.bank == 'PASARGAD' and getattr(
            settings, 'POS_BRIDGE_URL', '') and not getattr(
            settings, 'POS_TEST_MODE', True):
...
def send_to_pos_serial(pos_device, amount):
    ...
    if pos_device.device_type == 'VERIFONE_VX520' and pos_device.bank == 'SAMAN':
        try:
            # ساخت داده‌های ارسالی
            command = (f"AMOUNT:{amount};TERMINAL:{pos_device.terminal_id}"
                       f";MERCHANT:{pos_device.merchant_id}\r\n")
